...
export default function TopHeader({ title = "Dashboard", onToggle }) {
    ...
    return (
        <header className="fixed top-0 right-0 left-0 lg:left-64 h-20 z-50 bg-white/80 dark:bg-[#041627]/80 backdrop-blur-md flex justify-between items-center px-4 md:px-10 border-b border-outline-variant/10 font-headline">
            <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg md:text-2xl font-extrabold tracking-tight text-primary dark:text-white uppercase truncate max-w-[150px] md:max-w-none">{title}</h2>
            </div>
            ...
            <div className="flex items-center gap-2 md:gap-6">
                ...
                <div className="flex items-center gap-2 md:gap-3 pl-2 md:pl-6 border-l border-outline-variant/30">
                    <div className="text-right hidden xs:block">
                        <p className="text-xs md:text-sm font-bold text-primary dark:text-white leading-none">{currentUser?.displayName?.split(' ')[0] || 'User'}</p>
